...
/**
 * REST controller for managing {@link com.application.foodapp.domain.Order}.
 */
@RestController
...
public class OrderResource {
    ...
    @GetMapping("/active-order-by-current-user")
    public ResponseEntity<OrderDTO> getActiveOrderByCurrentUser() {
        log.debug("REST request to getActiveOrderByCurrentUser");
        Optional<OrderDTO> orderDTO = orderService.findFirstByCurrentUserAndStatusIsActive();
        return ResponseUtil.wrapOrNotFound(orderDTO);
    }

    @GetMapping("/get-total-quantity")
    public ResponseEntity<Long> getTotalQuaEntity() {
        log.debug("REST request to getTotalQuaEntity");
        return ResponseEntity.ok(orderService.getTotalQuantity());
    }
